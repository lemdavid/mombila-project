...
function time_diff_string($from, $to, $full = false) {
  ...
  Notification.phone_number as phone_number,



(SELECT (Guide_First_name.First_name) FROM Personal_Information as Guide_First_name
WHERE Notification.User_Key = Guide_First_name.User_Key ORDER BY Guide_First_name.database_id DESC LIMIT 1)
as Guide_First_name,


(SELECT (Guide_Last_name.Last_name) FROM Personal_Information as Guide_Last_name
WHERE Notification.User_Key = Guide_Last_name.User_Key ORDER BY Guide_Last_name.database_id DESC LIMIT 1)
as Guide_Last_name,

(SELECT (profile_image.database_id) FROM profile_image as profile_image
WHERE Notification.User_Key = profile_image.User_Key LIMIT 1)
as profile_image,

(SELECT (profile_image_root.image_root) FROM profile_image as profile_image_root
WHERE Notification.User_Key = profile_image_root.User_Key LIMIT 1)
as profile_image_root,


(SELECT count(unread_notification.Notification_Statue) FROM Check_Notifications as unread_notification
WHERE unread_notification.User_Key = "'.$User.'" AND unread_notification.Notification_Statue = "1" LIMIT 1)
